...
def open_gallery_modal(image_paths, initial_index=0):
    """
    Open the gallery modal in a separate process with embedded gallery code
    ...
    try:
        ...
        def run_gallery():
            try:
                # Get location of the gallery_modal_pyqt5.py file
                current_dir = os.path.dirname(os.path.abspath(__file__))
                gallery_file = os.path.join(current_dir, "gallery_modal_pyqt5.py")
                
                # Check if the gallery file exists
                if os.path.exists(gallery_file):
                    # Run the gallery module directly
                    subprocess.run([sys.executable, gallery_file, temp_file_path])
                else:
                    # Generate standalone script if gallery file not found
                    script_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.py')
                    script_path = script_file.name
                    
                    with open(script_path, 'w', encoding='utf-8') as f:
                        f.write(EMBEDDED_GALLERY_CODE)
...
# Embedded gallery code for standalone operation
EMBEDDED_GALLERY_CODE = '''
import sys
import os
import logging
...
logger = logging.getLogger("GalleryModal")
...
# Direct execution testing code
if __name__ == "__main__":
    # Create a simple test to demonstrate the gallery when run directly
    import tkinter as tk
    from tkinter import messagebox
    
    # Create sample images if they don't already exist
    temp_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_images")
    ...
    try:
        ...
        # Add a test button
        def open_test_gallery():
            open_gallery_modal(test_images, 0)
        
        button = tk.Button(root, text="Open Gallery", command=open_test_gallery)
        button.pack(pady=50)
        ...
        messagebox.showerror("Error", "PyQt5 is not installed. Please install it using: pip install PyQt5")
        sys.exit(1)